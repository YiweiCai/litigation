...
-->
	...
	<div class="main">
		...
		<div style="text-align: center;">
			来源：${uv.courtroom } &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			发布时间：${uv.issueTime }<br />

			<div class="source" id="a1" style="text-align: center;"></div>
			<font id="font" style="color:red;display:none;">您的浏览器版本过低，视频不兼容或加载时间较长！</font>
		</div>
		 
		<div class="detailtext">
			<div class="titlebg">案情介绍</div>
			<p>
				<strong>案由：</strong>${uv.brief }<br /> <strong>开庭时间：</strong>${uv.time
				}<br /> <strong>开庭地点：</strong>${uv.site}<br /> <strong>基本案情：</strong>${uv.videoIntro
				}
			</p>
			...
			<p>
				<strong>审判长：</strong>${uv.firestJudge }<br /> <strong>审判员：</strong>${uv.judger
				}<br />
				<c:if test="${uv.juror!=''&&uv.juror!=null }">
					<strong>陪审员：</strong>${uv.juror }<br />
				</c:if>
				<c:if test="${uv.agencyJudger!=''&&uv.agencyJudger!=null }">
					<strong>代理审判员：</strong>${uv.agencyJudger }<br />
				</c:if>
				<c:if test="${uv.societyJudger!=''&&uv.societyJudger!=null }">
					<strong>社会审判员：</strong>${uv.societyJudger }<br />
				</c:if>
			</p>
			<!--  
<div class="titlebg">庭审笔录</div>
