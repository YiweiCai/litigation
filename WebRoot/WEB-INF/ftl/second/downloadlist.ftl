					...
					<ul class="downlist">
						<#if  articleList?? && articleList?size gt 0>
							<#list articleList as article>
								<li><span title="${article.title}">${article.title}</span><a target="_blank" href="${ctx}/cms/article_download.do?articleId=${article.id}" title="点击下载">---- 点击下载</a></li>
							</#list>
						 <#else>
							<li>暂无${channel.channelName}信息</li>
						 </#if>
					</ul>
					<div class="pages">
