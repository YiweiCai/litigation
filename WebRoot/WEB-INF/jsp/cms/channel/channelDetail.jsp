...
<my:head lhgdialog="true">
   ...
   <script type="text/javascript">
        $(function() {
			...
			var isDynamicUrl = $("#isDynamicUrl").val();
			if(isDynamicUrl=='1'||isDynamicUrl=='2'||isDynamicUrl=='3'){
		...
		<tr>
			<td>
				<div align="center" id="uldiv">
					<ul>
						<li id="current"><a href="${ctx4ej }/cms/channel_update.do?id=${id }"><span>本栏目信息</span></a></li>
...
<form method="post" action="${ctx4ej }/cms/channel_save.do" id="channelForm">
	...
	<table width="100%" class="ftable"id="ftable">
		...
		<tr>
		  <th width="15%"><font color="red">*</font>是否为动态访问链接（非维护人员不可填）：</th>
		  <td>
		    <select id="isDynamicUrl" name="isDynamicUrl" align="left" onblur="isDynamic();">
		    	<option value="0" <c:if test="${isDynamicUrl=='0'||empty isDynamicUrl }"> selected="selected"</c:if> >否</option>
		    	<option value="1" <c:if test="${isDynamicUrl=='1' }"> selected="selected"</c:if>>本网站链接</option>
		    	<option value="2" <c:if test="${isDynamicUrl=='2' }"> selected="selected"</c:if>>外网链接</option>
		    	<option value="3" <c:if test="${isDynamicUrl=='3' }"> selected="selected"</c:if>>本网站链接且不隐藏文章发布</option>
		    </select>
		  </td>
		</tr>
		<tr id="hiddenUrl">
		  <th width="15%"><font color="red">*</font>访问链接地址（非维护人员不可填）：</th>
		  <td>
		    <input type="text" maxlength="2000" id="dynamicUrl" name="dynamicUrl" value="${dynamicUrl }" maxlength="2000" align="left"/>
		  </td>
		</tr>
