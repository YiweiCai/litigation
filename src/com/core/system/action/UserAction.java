...
public class UserAction extends BaseUploadFileAction<SysUser>{
	...
	public String save() throws Exception{
		...
		if(StringUtils.isBlank(id)){
            ...
            user.setOrganization(deptService.get(deptId).getOrganization());
    ...
    public String getCurrentUserByAjax() throws Exception{
        SysUser currentUser = Sessions.getSysUser();
        if (currentUser == null || StringUtils.isEmpty(currentUser.getId())) {
            Renders.renderJson(new JsonResult("1", "当前未登录"));
        } else {
            ...
            Renders.renderJson(new JsonResult("0", userInfo));
        }
    ...
    /**
     * 
     * <li>功能描述：检查用户名是否存在，存在返回true，不存在返回false，Ajax调用</li>
     * 
     * @return
    ...
    public String checkLoginName() throws Exception {
        String loginName = Servlets.getStringParameter("loginName", "");
        String hql = "from SysUser where loginName='" + loginName + "' and id<>'" + id + "'";

        int count = this.userService.findCount(new HqlBuilder(hql));

        Renders.renderJson(new JsonResult(count > 0));
    ...
    /**
     * 
     * <li>功能描述：检查邮箱是否存在，存在返回true，不存在返回false，Ajax调用</li>
     * 
     * @return
     * @throws Exception String
     * @author xcl
    ...
    public String checkEmail() throws Exception {
        String email = Servlets.getStringParameter("email", "");
        String hql = "from SysUser where mailbox='" + email + "' and id<>'" + id + "'";
